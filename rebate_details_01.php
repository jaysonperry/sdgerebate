<!doctype html>
<!--[if lt IE 7]> <html class="ie6 oldie"> <![endif]-->
<!--[if IE 7]>    <html class="ie7 oldie"> <![endif]-->
<!--[if IE 8]>    <html class="ie8 oldie"> <![endif]-->
<!--[if gt IE 8]><!-->
<html class="">
<!--<![endif]-->
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>San Diego Gas &amp; Electric&reg; Rebates for Home Energy Saving Products | - Lighting Rebates -</title>
<link href="css/boilerplate.css" rel="stylesheet" type="text/css">
<link href="/css/layout_details.css" rel="stylesheet" type="text/css">
<link href="/css/global_styles.css" rel="stylesheet" type="text/css" media="all">

<script src="/js/timeout-medium.js"></script>
<!-- 
To learn more about the conditional comments around the html tags at the top of the file:
paulirish.com/2008/conditional-stylesheets-vs-css-hacks-answer-neither/

Do the following if you're using your customized build of modernizr (http://www.modernizr.com/):
* insert the link to your js here
* remove the link below to the html5shiv
* add the "no-js" class to the html tags at the top
* you can also remove the link to respond.min.js if you included the MQ Polyfill in your modernizr build 
-->
<!--[if lt IE 9]>
<script src="//html5shiv.googlecode.com/svn/trunk/html5.js"></script>
<![endif]-->
<script src="js/respond.min.js"></script>
</head>
<body oncontextmenu="return false;">
<div class="gridContainer clearfix">
  <div id="outerWrapper" class="fit-screen">
    <div id="header">
      <h1 class="app-title">Home Savings Center</h1>
      <!--<div class="branding pull-right">
   	  	<img src="/images/sdge-logo-header.png" alt="San Diego Gas and Electric Company Logo Image">
      </div>
      <div class="pull-stop"></div>-->
    </div>
    <div id="mainContent" class="detail-main-content">
      <h1>Lighting</h1>
      <h2 class="savings-amount">Reduced Pricing made possible by SDG&amp;E<sup>&reg;</sup>.</h2>
      <h2>on select energy-efficient  lighting products.   All reduced price LED products are high CRI<span class="footnote-charm">*</span> of +90.</h2>
      <ul class="list-ul list-style-none rebate-features">
        <li>
        	The Color Rendering Index (CRI) is a scale from 0 to 100.
          <ul class="list-ul list-style-none footnote">
            <li class="no-content">
       			<span class="text-eternal-blue">*</span> The higher the CRI, the better the color rendering ability. CRI measures how accurately a light source displays colors. </li>
          </ul>
        </li>
      </ul>
      <div class="light-bulb-spread"><img src="/images/Cree-60w-A19-TW-LED.png" alt="A19 LED"><img src="/images/Cree-LTG-CR-Series-Downlights.png" alt="Retrofit LED"><img src="/images/ecosmart-br30-cfl.png" alt="BR30 LED"></div>

    </div>
    <div id="sidebarRight">
    	<div class="text-box-1">
          <h3>Field Guide</h3>
          <img class="drop-shadow-object img-center" src="/images/Lighting-sticker_2014.png" alt="Light Bulb Shelf Sticker">
					<p class="field-guide-text">Look for <strong>this label</strong> identifying <q>&ldquo;<em>qualifying light bulbs</em>&rdquo;</q> on the lighting aisle.</p>
            
        </div>
        <div class="sidebar-btn-wrapper">
        	<a class="btn btn-sm" href="/menu.php">back</a>
    	</div>
    </div>
  </div>
    <div id="footer" class="site-footer">
      <div class="rule-horizontal-bullets">&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;</div>
      <!--<p class="slogon-footer">Save money, energy, and resources</p>-->
    </div>
</div>

</body>
</html>
